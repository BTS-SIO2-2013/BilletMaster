...
$.extend($.datepicker._defaults, {
    ...
    'stepHeures': 1, // Number of heures to step up/down
...
Timepicker.prototype = {
    ...
    show: function (input)
    {
        ...
        var stepHeures   = parseInt($.datepicker._get(inst, 'stepHeures'), 10)   || 1;

        if (60 % stepMinutes != 0) { stepMinutes = 1; }
        if (24 % stepHeures != 0)   { stepHeures   = 1; }

        $('#heureSlider').slider('option', 'max', 24 - stepHeures);
        $('#heureSlider').slider('option', 'step', stepHeures);
    ...
    update: function (fd)
    {
        var curTime = $('#' + this._mainDivId + ' span.fragHeures').text()
                    + ':'
    ...
    resize: function ()
    {
        ...
        $('#heureSlider').css('height',   this.tpDiv.height() - (3.5 * hdrHeight));
    ...
    _generateHtml: function ()
    {
        ...
        html += '<span class="fragHeures">08</span><span class="delim">:</span><span class="fragMinutes">45</span> <span class="fragAmpm"></span></div></div><table>';
        html += '<tr><th>Heure</th><th>Minute</th></tr>';
        html += '<tr><td align="center"><div id="heureSlider" class="slider"></div></td><td align="center"><div id="minuteSlider" class="slider"></div></td></tr>';
        html += '</table>';
        ...
        $('#heureSlider').slider({
            ...
            slide: function(event, ui) {
                self._writeTime('heure', ui.value);
            },
    ...
    _writeTime: function (type, value)
    {
        if (type == 'heure') {
            ...
            $('#' + this._mainDivId + ' span.fragHeures').text(value);
        }
    ...
    _setTime: function (type, value)
    {
        ...
        if (value < 0)    value = 0;
        if (value > 23 && type == 'heure')   value = 23;
        if (value > 59 && type == 'minute') value = 59;

        if (type == 'heure') {
            $('#heureSlider').slider('value', value);
        }
